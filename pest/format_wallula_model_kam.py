...
# Katie added:
# (1) adjusted obs weights to be normalized obs weights (i.e., sum to 1) for improved model goodness of fit comparisions;
# (2) Error check to see if simulation made it to the final time,
...
pressure_obs_name = 'Walulla_Post_Injection_Hydraulic_Test_Field_Data.csv'
...
elif str(sim_output.TM.iloc[-1]) == 'nan':
    sim_error = True
    print('Simulation did not reach required time for PEST comparision')
else:
    sim_error = False
...
#observations.append(proxy_secondary_obs)
...
plagioclase        none  relative    8.03e-08      8.0e-9   8.0e-7 dgr             1.0000        0.0000      1
* observation groups
obsgroup
* observation data
...
with open("wallula_output_processed.txt", "w") as txt_file:
    if sim_error:
        model_output_error = np.ones(len(model_output.tolist()))*-999
